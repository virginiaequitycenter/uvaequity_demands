...
function drawdemos(data) {
    ...
    demosvgs.append("title")
        .attr("id", "title")
        .attr("lang", "en")
        .text((d) => "Circular bar grap of " + d.key + ". " +
            d.values[[0]].Demographic + " have " + d.values[[0]].Count + " contributions. " +
            d.values[[1]].Demographic + " have " + d.values[[1]].Count + " contributions. " +
            d.values[[2]].Demographic + " have " + d.values[[2]].Count + " contributions. " +
            d.values[[3]].Demographic + " have " + d.values[[3]].Count + " contributions. " +
            d.values[[4]].Demographic + " have " + d.values[[4]].Count + " contributions. " +
            d.values[[5]].Demographic + " have " + d.values[[5]].Count + " contributions. " +
            d.values[[6]].Demographic + " have " + d.values[[6]].Count + " contributions. "
    ...
    var x = d3.scaleBand()
        ...
        .domain(DemosNested[[0]].values.map(function (d) {
    ...
    var y = scaleRadial()
        ...
        .domain([0, d3.max(DemosNested[[1]].values.map(function (d) {
            return +d.Percent;
        }))]);
    ...
    circlecontainers.append("g")
        ...
        .attr("d", d3.arc() // imagine your doing a part of a donut plot
            ...
            .endAngle(function (d) {
                return x(d.Demographic) + x.bandwidth();
            })
            .padAngle(.25)
    ...
    totalcircle
        ...
        .attr("text-anchor", function (d) {
            return (x(d.Demographic) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI ? "end" : "start";
        })
        .attr("transform", function (d) {
            return "rotate(" + ((x(d.Demographic) + x.bandwidth() / 2) * 180 / Math.PI - 90) + ")" + "translate(" + (y(-5)) + ",0)";
        })
        .append("text")
        .text(function (d) {
            return (d.Demographic)
        ...
        .attr("transform", function (d) {
            return (x(d.Demographic) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI ? "rotate(180)" : "rotate(0)";
        })
